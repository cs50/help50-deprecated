...
def help(lines):
    ...
    if matches:
        ...
        if array:
            reponse = ["Careful, it looks like you are trying to index into the first position of the array '{}' on line {} of '{}'".format(array, matches.group(2), matches.group(1))]
        else:
        ...
        response.append("But remember that arrays in C are 0 indexed, so in order to get the first element, use [0] instead of [1].")
    if len(lines) is 3:
        return (3, response)
    return (1, response)
