...
def help(lines):
    ...
    # $ clang foo.c
    # /tmp/foo-1ce1b9.o: In function `main':
    # foo.c:6:20: error: 'main' must return 'int'
    #     void main()
    #     ^
    matches = re.search(r"^([^:]+):(\d+):\d+: error: 'main' must return 'int'", lines[0])
    if matches:
        response = [
            "It looks like you've declared your main fuction with a return type that isn't an int",
            "Make sure that your main function is declared as either 'int main(void)' or 'int main(int argc, char* argv[])' (or 'int main(int argc, string argv[])' if you're using the cs50 library)"
        ]
        return (1, response)
